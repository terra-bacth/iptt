...
window.filterTasks = function(filterType) {
  ...
  // Update button active state in filterBar
  const filterBtns = ["filterAll", "filterPending", "filterCompleted", "filterDelayed"];
  filterBtns.forEach(id => {
    const btn = document.getElementById(id);
    if (btn) btn.classList.remove("active");
  });

  const activeId = "filter" + (filterType.charAt(0).toUpperCase() + filterType.slice(1));
...
function renderGrid() {
  ...
  executionData.forEach(scope => {
    ...
    /* ===== GROUP TASKS WITH FILTER ===== */

    const groupedTasks = {};

    scope.tasks.forEach(t => {
      // Apply currentFilter
      if (currentFilter === "pending" && t.status === "Completed") return;
      if (currentFilter === "completed" && t.status !== "Completed") return;
      if (currentFilter === "delayed" && !(Number(t.delay_days) > 0 || t.status === "Blocked")) return;
    ...
    if (Object.keys(groupedTasks).length === 0) {
      html += `
        <div style="padding: 32px 20px; text-align: center; color: var(--muted); background: var(--surface-card); border-radius: var(--radius); border: 1px solid var(--border); margin: 16px 0;">
          No tasks match the filter "<strong>${currentFilter}</strong>" for this scope.
        </div>
      `;
    }
    ...
    Object.keys(groupedTasks).forEach(group => {
        ...
        // Auto-expand group if filter is active
        const autoOpen = currentFilter !== "all";
        ...
        html += `
          <div class="task-group">
            ...
            <div class="group-content" style="${autoOpen ? 'display:block;' : 'display:none;'}">
              <table>
                <tr>
                  <th>Task</th>
                    <th>P.Start</th>
