...
def get_programme_executive_summary(
    ...
    # -----------------------------------
        ...
        delays_str = f"{total_delays} delayed execution item{'s' if total_delays != 1 else ''}"
        projects_str = f"{total_projects} active project{'s' if total_projects != 1 else ''}"
        executive_summary = (
            f"Programme '{programme.name}' currently manages {projects_str}. "
            f"Average programme health is {avg_health}% with overall progress tracking at {avg_progress}%. "
            f"The dominant execution stage across the programme is '{dominant_stage}'. "
            f"A total of {delays_str} are recorded across all projects. "
            f"Current programme operational assessment is '{programme_status}'."
        )
