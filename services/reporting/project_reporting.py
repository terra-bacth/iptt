...
def get_project_executive_summary(
    ...
    delay_str = f"{delayed_count} delayed execution item{'s' if delayed_count != 1 else ''}"
    executive_summary = (
        f"Project health is currently tracking at {kpis['health']}% with an overall progress of {kpis['progress']}%. "
        f"The majority of delivery nodes are currently in the '{dominant_stage}' stage. "
        f"A total of {delay_str} have been recorded, with the most delayed stage observed in '{bottleneck}'. "
        f"Current operational assessment indicates that {health_comment.lower()}."
    )
