...
def search_index_api(request: Request):
    user = request.session.get("user")
    if not user:
        return []

    from models import Programme, Project, ProjectAssignment
    with get_db_ctx() as db:
        ...
        if role == "pm":
            assigned_project_ids = [
                p[0] for p in db.query(ProjectAssignment.project_id).filter(
                    ProjectAssignment.user_id == user_id
                ).all()
            ]
            projects = db.query(Project).filter(Project.id.in_(assigned_project_ids)).all()
            programme_ids = list(set([p.programme_id for p in projects if p.programme_id]))
            programmes = db.query(Programme).filter(Programme.id.in_(programme_ids)).all()
        else:
        ...
        for prog in programmes:
            items.append({
                ...
                "category": "Programmes",
                "icon": "📦",
                "url": f"/programme/{prog.id}/executive-dashboard"
            })
        ...
        for proj in projects:
            items.append({
                ...
                "category": "Projects",
                "icon": "⚡",
                "url": f"/project/{proj.id}/executive-dashboard"
            })
            if role != "viewer":
                items.append({
                    "title": f"{proj.name} (Execution)",
                    "category": "Execution",
                    "icon": "🛠",
                    "url": f"/project/{proj.id}/execution"
                })
...
def export_audit_logs(
    ...
    from services.reporting.audit_export import (
        generate_audit_csv,
    ...
    with get_db_ctx() as db:
        query = db.query(ExecutionAuditLog).order_by(
            ExecutionAuditLog.timestamp.desc()
        )
        if scope_id:
            query = query.filter(ExecutionAuditLog.scope_id == scope_id)
        if limit:
    ...
    if fmt in ("xlsx", "excel"):
        content = generate_audit_excel(logs)
        media_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        filename = f"audit_logs_{timestamp_str}.xlsx"
    elif fmt == "json":
        content = generate_audit_json(logs)
        media_type = "application/json"
        filename = f"audit_logs_{timestamp_str}.json"
    else:  # default CSV
        content = generate_audit_csv(logs)
...
def export_executive_pdf(
    ...
    filename = f"Executive_Report_Project_{project_id}.pdf"
...
async def programme_dashboard_pdf(
    ...
    filename = f"programme_dashboard_{programme_id}.pdf"
    disposition = "attachment" if download == 1 else "inline"
